...
public class BaseClass extends MecanumDrive {
   ...
   // double intakerotpose=0,gearboxpose=0;
    ...
    double arm_angle_idle=-6,arm_angle_preintake=12,arm_arngle_intake=7,arm_angle_sampleouttake=107,arm_angle_specintake=197,arm_angle_specouttake=33;
    ...
    double lefthandle_sampleouttake=0.64,lefthandle_specintake=0.24,lefthandle_specouttake=0.64,lefthandle_start=0;
    ...
    double righthandle_sampleouttake=0.36,righthandle_specintake=0.36,righthandle_specouttake=0.36,righthandle_start=1;
    ...
    int pidf_intake_up=20,pidf_sampleintake=1,pidf_sampleouttake=2, pidf_spinintake=3,pidf_specouttake=4, pidf_idle=5,pidf_outtake_down=1,pidf_outtake_up=3, pidf_intake_idle = 4,
            ...
            pidf_outtake_spec_down = 9, pidf_outtake_spec1 = 10 , pidf_outtake_up2 = 11, pidf_hang0=11,pidf_hang1=12,pidf_hang2=13,
    ...
    public void  intake_smooth_shift(double sticky) {
        ...
        if(sticky<-0.5) {linearslide(slide_intakemax,slidev0-100) ;flag[smooth_adj]=true;return ;}
        if(sticky>0.5)  {linearslide(slide_preintake,slidev0-100); flag[smooth_adj]=true;return;}
        if(flag[smooth_adj]) {
...

  {
...

    {
        if(flag[prespecintake]&& slidePos <slide_rotate){  //slide roataiton target
            pidfsetting(arm_angle_specintake-5);
        ...
        if (Math.abs(arm_angle-arm_angle_specintake)<20)
        {
           ...
           pidf_index=pidf_specintake;
            pidfsetting(arm_angle_specintake+4);
    ...
    public void specouttake() {
        ...
        move(1.0);
        while(Op.opModeIsActive() && dist>150) {
...
//            Intake.setPower(0.4);
            ...
            delay(300);
            stop_drive();
            pidfsetting(rotate_spec_out);//pre postion.
             move(0.3);
//            Intake.setPower(0);
            delay(200);
...
//        Intake_handle.setPosition(handle_specimen_outtake - 0.04);
        ...
        pidfsetting(rotate_spec_out + 600);
        delay(300); //250
//        Intake.setPower(-0.8);
        pidfsetting(rotate_spec_out + 600);
        delay(150); //150
...
//           Intake_rot.setPosition(handlerot_intake);
         ...
         //  pidftable[pidf_aouttake_up2][pp]=0.001;  pidftable[pidf_aouttake_up2][ii]=0.00018;  pidftable[pidf_aouttake_up2][dd]=0.0000;//0
           ...
           pidftable[pidf_specintake][pp]=0.0021;  pidftable[pidf_specintake][ii]=0.00002;  pidftable[pidf_specintake][dd]=0.000;
           ...
           pidftable[pidf_idle][pp]=0.001;  pidftable[pidf_idle][ii]=0;  pidftable[pidf_idle][dd]=0.000;
...
//            Intake_rot.setPosition(handlerot_intake);
            pidfsetting(rotate_spec_first+200);
            delay(400);
...
//         Intake_handle.setPosition(0.28);
         pidfsetting(rotate_spec_first-500);
            delay(150); //250
//            Intake.setPower(-0.9);
            delay(150); //150
//            Intake.setPower(0);
...
//            Intake_handle.setPosition(0.20);
            pidfsetting(rotate_spec_first-400);
